...
type ChangePasswordRequest2 struct {
...
func GetEmailPassword2(c *gin.Context) {
	userEmail := c.Query("email")

	var company CompanyPassword
	// Realiza la consulta para obtener el email y la contraseña de la empresa
	err := db.DB.QueryRow("SELECT email, password FROM customer WHERE email = $1", userEmail).Scan(&company.Email, &company.Password)
	if err != nil {
...
func ChangeCustomerPassword(c *gin.Context) {
	...
	_, err = db.DB.Exec("UPDATE customer SET password = $1 WHERE email = $2", hashedPassword, req.Email)
	if err != nil {
		if err == sql.ErrNoRows {
			c.JSON(http.StatusNotFound, gin.H{"error": "Cliente no encontrada"})
		} else {
